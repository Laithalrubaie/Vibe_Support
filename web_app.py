...
context_list = [
    ...
    for r in res.data]                                    for r in res.data]
            ...
            context_text = "فشل في إنشاء المتجهات للسؤال."

        ai_prompt = f"""
        أنت مساعد خبير لموظفي الدعم الفني في منصة تعليمية عراقية. مهمتك هي إعطاء حلول دقيقة بناءً على "المعلومات المسترجعة" فقط.
        
        # قواعد العمل
        1. **التعامل مع الأسئلة العامة أو غير الواضحة:** إذا كان سؤال الموظف عامًا أو غير واضح (مثال: "الطالب عنده مشكلة")، لا تقدم حلاً مباشرًا. بدلاً من ذلك، اطلب من الموظف تقديم تفاصيل إضافية (مثل: "ما هو نوع المشكلة؟ Packs؟ الدفع؟"، "في أي صف؟").
        2. **التعامل مع المعلومات غير المتوفرة:** إذا لم تتضمن "المعلومات المسترجعة" حلاً مباشرًا للسؤال المطروح، أجب بـ "هذه المعلومة غير متوفرة عندي حالياً". لا تحاول تأليف حل من عندك.
